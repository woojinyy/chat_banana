...
<a href="javascript:boardDetail('<%=rMap.get("BM_NO") %>')" style="text-decoration:none;color:#000000">        		
        		...
        		<td>
        		<a href="javascrpit:fileDown('<%=rMap.get("BS_FILE")%>')"style="text-decoration:none:color:#000000">
        		<%=rMap.get("BS_FILE")%>	
        		</a>
        		</td>
...
<!-- 페이지 네이션 추가 시작 -->
	...
	<%
	//페이지네비게이션이 필요한 페이지 파라미터로 페이지 경로를 받도록 처리
	String pagePath="boardList.st3";
	//인스턴스화 할 때 생성자 파라미터로 페이지 처리에 필요한 변수들을 초기화 하도록 
	PageBar pb = new PageBar(numPerPage,size,nowPage,pagePath);
	//페이징 처리에 필요한 <a>태그들에 대한 문자열을 만들어서 getter메소드(getPageBar())를 호출하면
	//페이징 처리에 필요한 문자열을 StringBuilder에 처리하였고 그 문자열을 toString()으로 읽어 와서 출력
